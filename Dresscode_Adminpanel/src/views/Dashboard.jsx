import { IndianRupee, Users, ShoppingBag, Activity, User } from 'lucide-react';
import StatCard from '../components/dashboard/StatCard';

const Dashboard = () => {
  const stats = [
    { title: 'Total Revenue', value: '₹45,231.89', icon: IndianRupee, trend: '+20.1%', trendUp: true },
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-12 duration-1000">
        <div className="lg:col-span-2 p-6 rounded-2xl bg-white/40 border border-gray-200/50 backdrop-blur-sm min-h-[400px]">
          <h2 className="text-xl font-bold text-black mb-4">Revenue Overview</h2>
          <div className="w-full h-[300px] flex items-center justify-center border-2 border-dashed border-gray-200 rounded-xl text-black">
            [Chart Area Placeholder]
          </div>
        </div>
        <div className="p-6 rounded-2xl bg-white/40 border border-gray-200/50 backdrop-blur-sm">
          <h2 className="text-xl font-bold text-black mb-4">Recent Activity</h2>
          <div className="space-y-4">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="flex items-center gap-4 pb-4 border-b border-gray-200/50 last:border-0 last:pb-0">
